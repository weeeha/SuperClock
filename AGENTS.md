...
- **Colour goes through three tiers; the admin's tier 3 is not built yet.** `src/styles/tokens.css` holds tier 1 ramps (raw steps, read only by tier 2) and tier 2 semantic roles, the only place a light value and a dark value are ever written: `--brand`, `--brand-ink`, the `--status-*` family, `--surface-*`, `--ink`, `--ink-muted`, `--line`, `--focus`, `--fill*`, `--sheet-ink`, plus the eight `--face-*` names, which kept their spelling and are roles themselves, not an alias layer over new names. Tier 3 is where a vendor vocabulary, the admin's shadcn names, would alias onto a role instead of holding a value; declare an alias only in the same change that gives it a real consumer, or it ships as a dead custom property the liveness gate has to ledger. The admin does not import this file yet. A component reads a utility or a role, never a ramp. A surface that does not change between modes needs foreground roles that do not change either: the quick-settings sheet stays dark in both palettes, so its text and toggle knob read `--sheet-ink` and `--fill-knob` rather than `--ink`/`--ink-muted`, which invert by design. Gated by `src/shared/token-tiers.test.ts` (both modes present, no role holds a literal, the eight `--face-*` values pinned to their pre-layer numbers, in both modes), `src/shared/token-contrast.test.ts` (every ink/surface pair at 4.5:1, both modes), and `src/shared/token-liveness.test.ts`, extended to this file and tier-aware so a ramp read only by its own role does not count as dead.
...
- **Tailwind v4** via `@tailwindcss/vite`; the kiosk's `src/index.css` imports `src/styles/tokens.css`, where its values now live (see the tier bullet above), and keeps its own `@theme` block for a handful of tokens whose job is to become Tailwind utilities (`--color-sheet` mints `bg-sheet`), not to hold the kiosk's theme values. Admin tokens still live separately in `src/admin/index.css`. No `tailwind.config.*`.
...
- **Two rule catalogues coexist after the 2026-09-06 merge, and consolidating them is an open decision.** `rules/superclock.json` (records with severity and a detector, run by `npm run check:rules`) came from the agentic design-system branch; `scripts/lib/rules.mjs` (R01 to R17, each `checkedBy` a script or test, or `unchecked` with a reason, printed by `npm run check:tokens`) came from the contracts branch. They overlap in intent and neither is wrong. Add a rule to `rules/superclock.json` unless it is one of R01 to R17. Tokens declared but read by nothing are ledgered in `UNCONSUMED_LEDGER` (`scripts/lib/token-liveness.mjs`, shrink-only) and gated by `src/shared/token-liveness.test.ts`.
...
- **The one-accent-quantity rule is declared as data but not rendered-checked** (`FCE-1`, `judgment`; each face meta's `accent` names the one quantity, so a second one is visible in `index/parts.toon`, but nothing renders the face to count it). **LVGL parity is now gated**: `src/shared/lvgl-parity.test.ts` diffs `slow-native/src/clock_face.c` against the Analog contract's spec, and the nine known mismatches sit in `PARITY_DRIFT_LEDGER` (shrink-only). Which renderer is right is a decision waiting on Nick, not a fix-forward.
...
- **Three default oranges coexist**: the kiosk CSS fallback `--color-accent: #ff8826` (src/index.css), the config default `DEFAULT_ACCENT = '#ff6b35'` (src/shared/types.ts, applied once config arrives), and the admin's `--primary: 20 96% 60%` (src/admin/index.css), which resolves to `#fb7837`. The third was found on 2026-09-06 while deriving values for the token layer. Unifying them changes on-glass color — it is a design decision, not a cleanup.
