...
- **The kiosk's mode axis exists but reaches nine tokens.** `apply-settings.ts` already toggles `html.dark` / `html.light` from the theme setting, falling back to the night window. Only the `--face-*` set flips on it. The chrome paints `#000` and `#fff` directly on `body`, and its twelve remaining colour spots are `bg-black`, `text-white/50`, `bg-white/25` and their siblings. A mode flip cannot reach a literal.
