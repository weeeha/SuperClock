...
**Verified:** `./scripts/gates.sh` green end to end, run in full for the first time on this sub-project's work: lint clean; `check:tokens` reports 51 semantic-zone files and 13 faces clean (7 legacy faces exempt), 21 tokens ledgered in `UNCONSUMED_LEDGER`, and 5 rules with no detector (R09, R12, R13, R14, R15; R11 is no longer among them); 987 tests across 68 files (this count includes docs-drift.test.ts's own per-token checks, which grew as this entry's AGENTS.md bullets added new backticked names for the gate to resolve against the tree); both-SPA production build, `build-info.json` stamped `07c23d2`. This total moves whenever a backticked token name is added to or removed from `AGENTS.md` or `.claude/rules/*.md`, because `docs-drift.test.ts` mints one test case per token; a later session comparing counts should expect that drift. Task 5's review round independently re-derived all seven of the proof surface's colour mappings four ways (static analysis, a standalone script, live `getComputedStyle` in both modes and both toggle states, and the compiled bundle); every alpha channel matched exactly and the mode-invariance held in the running page, not just in the source.
